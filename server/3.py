...
class CustomTogetherLLM:
    ...
    def _call_api(self, prompt):
        ...
        data = {"model": self.model, "prompt": prompt, "max_tokens": self.max_tokens, "temperature": self.temperature, "top_p": 0.95, "stop": ["<|endoftext|>"]}
...
class StarCodeCompletion(BaseTool):
    ...
    def _run(self, code_context: str) -> str:
        ...
        data = {"model": "togethercomputer/StarCoder", "prompt": code_context, "max_tokens": 500, "temperature": 0.3, "top_p": 0.95, "stop": ["<|endoftext|>"]}
...
class StarBugDetection(BaseTool):
    ...
    def _run(self, code: str) -> str:
        ...
        Analysis:
        """
        data = {"model": "togethercomputer/StarCoder", "prompt": prompt, "max_tokens": 700, "temperature": 0.2, "top_p": 0.95, "stop": ["<|endoftext|>"]}
        response = requests.post(STARCODER_API_ENDPOINT, headers=headers, data=json.dumps(data))
        if response.status_code == 200:
           result = response.json()
...
class StarCodeTesting(BaseTool):
    ...
    def _run(self, code: str) -> str:
      ...
      prompt = f"""
        ...
        Test code:
        """
      data = {"model": "togethercomputer/StarCoder", "prompt": prompt, "max_tokens": 800, "temperature": 0.2, "top_p": 0.95, "stop": ["<|endoftext|>"]}
      response = requests.post(STARCODER_API_ENDPOINT, headers=headers, data=json.dumps(data))
      if response.status_code == 200:
        result = response.json()
...
class SimpleAgent:
    def __init__(self, name, system_message, all_agents=None):
        ...
        self.all_agents = all_agents or {}  # Store reference to all agents
    ...
    def _manager_run(self, message, history):
        # Determine which agent to delegate to based on message content
        prompt = f"""
        ...
        # Parse response to extract agent name and plan
        try:
    ...
    def _specialized_run(self, message, history):
        ...
        # If the agent has tools and the response indicates tool usage
...
def setup_simple_agents():
    ...
    # First create all agents without references
    ...
    # Now update all agents with references to each other
...
def integrate_tools(agents):
    ...
    # rag_query_tool = {"name": "rag_query", "description": "Query the RAG system for information", "func": rag_query_tool_instance._run}
    # rag_loader_tool = {"name": "rag_loader", "description": "Load documents into the RAG system", "func": rag_loader_tool_instance._run}
